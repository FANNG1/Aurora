...
import com.datastrato.aurora.util.MapUtils;
...
public abstract class Config {
  ...
  private static final String CONFIG_PREPEND = "aurora.";
  ...
  public Config loadFromFile(String name) throws Exception {
    String confDir =
        Optional.ofNullable(System.getenv("AURORA_CONF_DIR"))
            .orElse(
                Optional.ofNullable(System.getenv("AURORA_HOME"))
                    .map(s -> s + File.separator + "conf")
                    .orElse(null));

    if (confDir == null) {
      throw new IllegalArgumentException("AURORA_CONF_DIR or AURORA_HOME not set");
    }
  ...
  public <T> void set(ConfigEntry<T> entry, T value) {
    if (entry.isDeprecated()
        ...
        && LOG.isWarnEnabled()) {
      LOG.warn(
          "Config {} is deprecated since Aurora {}. {}",
          entry.getKey(),

...
#
#set -ex
USAGE="-e Usage: bin/aurora.sh [--config <conf-dir>]\n\t
...
if [[ "$1" == "--config" ]]; then
  ...
  if [[ ! -d "${conf_dir}" ]]; then
    ...
    export AURORA_CONF_DIR="${conf_dir}"
  fi
  shift
fi
...
function check_process_status() {
  local pid=$(found_aurora_server_pid)

  if [[ -z "${pid}" ]]; then
    echo "Aurora Server is not running"
  else
    echo "Aurora Server is running[PID:$pid]"
  fi
}

function found_aurora_server_pid() {
  ...
  if ! kill -0 ${RUNNING_PIDS} > /dev/null 2>&1; then
    echo "Aurora Server running but process is dead"
  fi

  echo "${RUNNING_PIDS}"
}

function wait_for_aurora_server_to_die() {
  timeout=10
  timeoutTime=$(date "+%s")
...
function start() {
  ...
  if [[ ! -z "${pid}" ]]; then
    if kill -0 ${pid} >/dev/null 2>&1; then
      echo "Aurora Server is already running"
      return 0;
    fi
  fi

  if [[ ! -d "${AURORA_LOG_DIR}" ]]; then
    echo "Log dir doesn't exist, create ${AURORA_LOG_DIR}"
    mkdir -p "${AURORA_LOG_DIR}"
  fi

  nohup ${JAVA_RUNNER} ${JAVA_OPTS} ${AURORA_DEBUG_OPTS} -cp ${AURORA_CLASSPATH} ${AURORA_SERVER_NAME} >> "${AURORA_OUTFILE}" 2>&1 &

  pid=$!
  if [[ -z "${pid}" ]]; then
    echo "Aurora Server start error!"
    return 1;
  else
    echo "Aurora Server start success!"
  fi
...
function run() {
  ${JAVA_RUNNER} ${JAVA_OPTS} ${AURORA_DEBUG_OPTS} -cp ${AURORA_CLASSPATH} ${AURORA_SERVER_NAME}
}

function stop() {
  local pid

  pid=$(found_aurora_server_pid)

  if [[ -z "${pid}" ]]; then
    echo "Aurora Server is not running"
  else
    wait_for_aurora_server_to_die
    echo "Aurora Server stop"
  fi
}

HOSTNAME=$(hostname)
AURORA_OUTFILE="${AURORA_LOG_DIR}/aurora-server.out"
AURORA_SERVER_NAME=com.datastrato.aurora.server.IcebergRESTServer

JAVA_OPTS+=" -Dfile.encoding=UTF-8"
JAVA_OPTS+=" -Dlog4j2.configurationFile=file://${AURORA_CONF_DIR}/log4j2.properties"
JAVA_OPTS+=" -Daurora.log.path=${AURORA_LOG_DIR} ${AURORA_MEM}"

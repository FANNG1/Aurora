...
if [[ -z "${AURORA_HOME}" ]]; then
  export AURORA_HOME="$(cd "${FWDIR}/.." || exit; pwd)"
fi

if [[ -z "${AURORA_CONF_DIR}" ]]; then
  export AURORA_CONF_DIR="${AURORA_HOME}/conf"
fi

if [[ -z "${AURORA_LOG_DIR}" ]]; then
  export AURORA_LOG_DIR="${AURORA_HOME}/logs"
fi

if [[ -f "${AURORA_CONF_DIR}/gravitino-env.sh" ]]; then
  . "${AURORA_CONF_DIR}/gravitino-env.sh"
fi

AURORA_CLASSPATH+=":${AURORA_CONF_DIR}"
...
function addEachJarInDir(){
  if [[ -d "${1}" ]]; then
    for jar in "${1}"/*.jar ; do
      AURORA_CLASSPATH="${jar}:${AURORA_CLASSPATH}"
    done
  fi
}
...
function addJarInDir(){
  if [[ -d "${1}" ]]; then
    AURORA_CLASSPATH="${1}/*:${AURORA_CLASSPATH}"
  fi
}

if [[ -z "${AURORA_MEM}" ]]; then
  export AURORA_MEM="-Xmx1024m"
fi
